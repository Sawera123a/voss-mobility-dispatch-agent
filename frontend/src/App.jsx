...
import { ArrowRight, Activity, RadioTower, TriangleAlert, CircleCheck, CircleX, RefreshCw, Info, X } from "lucide-react";
import "./App.css";
  ...
  return (
    <div className={`zone-card zone-card--${status}`}>
      <div className="zone-card__head">
        <span className="zone-card__name">{zone.replace("Zone_", "Zone ")}</span>
        <span className={`zone-card__badge zone-card__badge--${status}`}>
          {status === "shortage" ? "SHORT" : status === "surplus" ? "SURPLUS" : "STABLE"}
        </span>
      </div>
      ...
      <div className="zone-gauge">
        <div className="zone-gauge__track">
          ...
          <div className="zone-gauge__demand-line" style={{ bottom: `${demandLinePct}%` }} />
        </div>
      ...
      <div className={`zone-card__plain zone-card__plain--${status}`}>{plainText}</div>
    </div>
  );
}
    ...
    <div className={`move-row ${approved ? "move-row--approved" : "move-row--rejected"}`}>
      ...
      <div className="move-row__body">
        <div className="move-row__path">
          ...
          <span className="move-row__from">{move.from_zone.replace("Zone_", "")}</span>
...
const runDispatchPlan = async () => {
    ...
    try {
      ...
      setError("Dispatch run failed. Check that the API and Ollama are running.");
    } finally {
  ...
  const zoneAverages = ZONES.reduce((acc, zone) => {
    ...
    const rows = forecast.filter((r) => r.zone === zone && hours.includes(r.hour));
    const avg = rows.reduce((s, r) => s + r.predicted_demand, 0) / (rows.length || 1);
    acc[zone] = avg;
    return acc;
  }, {});
    ...
    <div className="console">
      <header className="console__topbar">
        ...
        <div className="console__status">
          <button className="btn btn--icon" onClick={() => setShowHelp(true)} title="What does this do?">
            <Info size={16} />
          </button>
          <span className={`status-pill ${apiHealthy ? "status-pill--ok" : "status-pill--down"}`}>
            <Activity size={13} />
        ...
        <div className="help-overlay" onClick={() => setShowHelp(false)}>
          <div className="help-card" onClick={(e) => e.stopPropagation()}>
            <div className="help-card__head">
              ...
              <button className="btn btn--icon" onClick={() => setShowHelp(false)}><X size={16} /></button>
            </div>
            <p>This screen predicts, for each area of the city, whether there will be
              <b> too few cars</b> for the number of rides expected, or
              <b> too many idle cars</b> sitting around.</p>
            <p>Press <b>"Run Dispatch Agents"</b> and an AI planner decides which idle
              cars should move to the areas that need them - so customers wait less,
              and cars don't sit empty.</p>
            <p>Every suggested move is double-checked by a safety rule before it's
              shown as approved, so a car can never be sent from an area that doesn't
              actually have spare cars.</p>
          </div>
        </div>
      )}
            ...
            return (
              <>
                ...
                <span className="summary-strip__item summary-strip__item--surplus">
                  {surplusCount} area{surplusCount === 1 ? "" : "s"} have spare cars
                </span>
              </>
            );
          })()}
      ...
      <main className="console__main">
        <section className="console__section">
          <div className="console__section-head">
            <h2>Zone Status <span className="console__section-sub">next 3 hours, predicted</span></h2>
            <button className="btn btn--ghost" onClick={loadForecast} disabled={loadingForecast}>
        ...
        <section className="console__section console__section--panel">
          <div className="console__section-head">
            <h2>Dispatch Plan <span className="console__section-sub">agent-generated, guardrail-checked</span></h2>
            <button className="btn btn--primary" onClick={runDispatchPlan} disabled={runningPlan}>
          ...
          {runningPlan && (
            <div className="thinking-banner">
              ...
              Demand Analyst and Dispatch Planner are reasoning locally via Mistral...
              ...
              <div className="plan-columns">
                <div className="plan-column">
                  ...
                  {plan.approved_moves.length === 0 && (
                    <div className="empty-state empty-state--small">No moves approved this cycle.</div>
                  )}
                ...
                <div className="plan-column">
                  ...
                  {plan.rejected_moves.length === 0 && (
                    <div className="empty-state empty-state--small">No rejections this cycle.</div>
                  )}
      ...
      <footer className="console__footer">
        Autonomous Ride Dispatch Agent &middot; LightGBM demand model + CrewAI agents (local Mistral) &middot; Sweden
      </footer>
    </div>
  );
}
